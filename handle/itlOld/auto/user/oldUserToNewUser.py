...
class OldUserToNewUser(object):
    ...
    def __init__(self):
        ...
        with open(OldUserToNewUser.__BASE_PATH + 'itl_zone_category.txt', 'r') as f:
    ...
    def handle(self):
        ...
        with open(OldUserToNewUser.__BASE_PATH + 'out2222.sql', 'w') as f:
            f.write('BEGIN;\n')
            ...
            for key in keys:
                if key not in already_exist_mobile:
                    ...
                    sql = self.base_sql.format(account=key, password=old_user['password'], name=str(old_user['name']).replace('"', ''),
                                               ...
                                               category_id=self.get_zone_category_id(old_user['zone_id'], old_user['type']),
                                               category_name=category_group[old_user['type']],
    ...
    def get_zone_category_id(self, zone_id, cat_id):
        return self.zone_category_group[zone_id + ',' + cat_id]
