...
class OldUserToNewUser(object):
    ...
    def handle(self):
        ...
        with open(OldUserToNewUser.__BASE_PATH + 'out.sql', 'a') as f:
            f.write('BEGIN;\n')
            ...
            for key in keys:
                ...
                sql = self.base_sql.format(account=key, password=old_user['password'], name=old_user['name'], identity_card=old_user['Idcard'], nickname=old_user['name'],
                                           sex=sex, mobile=key, avatar_url=old_user['avtar'], address=old_user['address'], email='',
                                           emergency_contact=old_user['emergency_name'], emergency_mobile=old_user['emergency_mobile'], last_login_time='',
                                           last_login_ip='', category_id=old_user['type'], category_name=category_name, company_id=old_user['cid'],
                                           zone_id=old_user['zone_id'], zone_ids=old_user['zone_id'], is_on_job='1', departure_time='', job_title_id='0',
                                           job_title_name='')
                f.write(sql)
            f.write('COMMIT;\n')
